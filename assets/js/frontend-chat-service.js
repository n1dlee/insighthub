...
class ChatService {
    ...
    async sendMessage(conversationId, content) {
      return this._fetchWithErrorHandling(`${this.baseUrl}`, {
        method: 'POST',
        headers: {
    ...
    async getConversation(conversationId) {
      return this._fetchWithErrorHandling(`${this.baseUrl}/${conversationId}`, {}, 'fetch conversation');
    }
  
    // Fetch all conversations for the logged-in user
    async getConversations() {
      return this._fetchWithErrorHandling(`${this.baseUrl}/conversations`, {}, 'fetch conversations');
    }
  
    // Start a new conversation with a specific participant
    async startConversation(participantId) {
      return this._fetchWithErrorHandling(`${this.baseUrl}/start`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
